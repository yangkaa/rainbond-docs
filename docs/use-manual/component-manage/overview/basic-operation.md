---
title: 组件基础操作
description: 应用上云平台组件基础生命周期操作说明文档
---
...
特别是对于第一次在 应用上云平台 启动的组件，尤其需要关注如下几点：

**1.组件启动或更新超时怎么办？**

目前 应用上云平台 对于异步任务确定了固定的超时时间，因此请注意，超时并不是失败，需要根据实际情况优化组件的配置，如果有超时，请按如下路径进行排查：
...
若是上述情况，请处理你的组件配置，组件运行过程中如果异常退出 应用上云平台 会自动守护并重新启动你的组件
