...
webapp-runner 可以在任何安装有 JRE 环境的系统中利用 Tomcat 将应用程序启动。使用 webapp-runner 不需要安装 Tomcat。它只是一个 jar 文件可以用`java`命令运行和配置。应用上云平台 运行 War 包、或 Maven 打包成 War 包的项目都默认使用 webapp-runner。

### 版本说明

> Webapp Runner 是基于 Tomcat server 的。默认 应用上云平台 支持的版本如下:
...
目前 应用上云平台 提供的 Webapp-runner 都支持 session 管理.
