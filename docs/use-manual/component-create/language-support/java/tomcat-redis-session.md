...
如果你使用 应用上云平台 源码部署 Java 项目，您可以通过如下两种方式实现 **配置 redis 实现 session 共享**:

### 使用 Webapp-Runner 或 Jetty-Runner

应用上云平台 使用 [webapp-Runner](./webapp-runner) 内嵌的 tomcat 或 jetty-Runner 内嵌的 jetty 实现服务器功能。在您不创建其他服务器情况下即可轻松将应用部署在 应用上云平台。通过以下步骤可实现 **配置 redis 实现 session 共享**。
...
1. 配置 [Procfile](../procfile)：将如下命令添加到您的 Procfile 中，并源码根目录下添加 Procfile。
   ...
   - 指定了监听端口，通过获取环境变量 \$PORT,此变量 应用上云平台 根据平台设置的服务端口进行自动注入
...
应用上云平台 提供使用定制 tomcat 容器来启动应用的方法。通过以下步骤可实现 **配置 redis 实现 session 共享**。
