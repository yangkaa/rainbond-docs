---
title: NodeJS前端语言
description: NodeJS前端语言类型应用上云平台支持规范介绍, 部署Vue React Angular源码部署到应用上云平台
---
...
应用上云平台 会根据源代码根目录是否有 `package.json` 和 `nodestatic.json` 文件来识别为 NodeJS 前端类项目，除此之外，源代码根目录下还必须存在以下两个文件之一（不可以同时存在）：

- `package-lock.json` 存在该文件时，应用上云平台 默认使用 npm 包管理器构建。

- `yarn.lock` 存在该文件时，应用上云平台 使用 yarn 包管理器构建。
...
- 执行以下构建命令，该命令也是 应用上云平台 nodejs 前端项目构建的默认命令：
...
环境准备阶段，需要将 应用上云平台 构建运行环境，和常用的本地构建运行环境尽量统一。比如 Node 版本、构建命令等。
...
`package.json` 文件中的 `scripts` 具备 CI 特征，应用上云平台 源码构建过程继承这些特性，比如：
...
项目编译完成后，应用上云平台 会默认使用 Nginx（1.14.2） 将前端项目运行起来。用户可以在源代码根目录下加入 `web.conf` 文件来指定 Nginx 的配置，该文件的作用是定义运行时参数，特别是对后台 API 的代理。没有此文件时，应用上云平台 会采用缺省配置。参考配置用例如下：
...
| BUILD_NODE_MODULES_CACHE |              `true`               | 指定是否开启构建缓存，在 应用上云平台 服务器磁盘性能低下时，指定为 `false` |
|    BUILD_NODE_VERBOSE    |              `false`              |      指定是否在构建日志中体现依赖列表（`npm ls` 或 `yarn list`）       |

### 启动命令配置

Nodejs 前端项目源码构建过程完成后，应用上云平台 会自动将服务组件运行起来，这需要我们事先指定服务组件的启动命令。

#### Procfile 规范

应用上云平台 通过源代码根目录下的 `Procfile` 文件来定义项目启动命令，`Procfile` 文件定义规范详见 [Procfile](../language-support/procfile) 。
...
示例[https://github.com/goodrain/应用上云平台-ui](https://github.com/goodrain/应用上云平台-ui.git)

### 常见问题解决

- 使用 git-revision-webpack-plugin 插件打包时报错。

  > 部分项目打包配置中设置了基于 git 来获取版本号，需要依赖源码目录中存在.git 文件。在 应用上云平台 中源码编译过程中使用的源代码不会存在.git 文件。因此不能使用 git 插件来获取版本。应用上云平台 平台上有版本管理，因此遇到这个问题请移除 webpack 配置文件中的 git 相关的插件。
...
- 平台上源码编译失败怎么办？
  ...
  >
  > （3）情景 3，构建日志提示 `No matching version found for Node: XXX` 或 `No matching version found for Yarn: XXX`，该信息提示 `package.json` 中 `scripts.engines` 定义的 Node 、 Yarn 版本暂时不被支持，请参考上述文档 `package.json` 配置版本部分。如果 应用上云平台 提供的版本的确不能满足项目需要，请联系好雨科技工程师添加。
  >
...
在此步骤中，你需要提供一个可用的NodeJS源码程序用来部署在应用上云平台平台上,此应用程序至少需要满足如下条件:

1. 本地可以正常运行的NodeJS项目
2. 源码程序必须托管在gitlab等相关git或者svn服务上
3. 源码程序根路径下必须存在`package.json`,用来管理NodeJS项目的依赖,是应用上云平台识别为NodeJS前端类语言的必要条件
4. 代码的根目录下必须有`nodestatic.json`文件，是应用上云平台识别为NodeJS前端类语言的必要条件
